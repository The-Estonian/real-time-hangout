...
func HandleLoadMoreMessages(w http.ResponseWriter, r *http.Request) {
	CorsEnabler(w)
	if r.Method == "OPTIONS" {
	...
	if err != nil {
		...
		if exists {
			...
			jsonData, err := json.Marshal(validators.GetMoreMessagesBeforeDB(user, messageAmount["count"], messageAmount["channelPartner"]))
			if err != nil {
				fmt.Println("Error marshaling callback in HandleGetMessages")
			}
			w.Write(jsonData)
