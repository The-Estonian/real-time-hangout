...
export const LoadMoreMessages = async (count, channelPartner) => {
  ...
  try {
    const response = await fetch('http://localhost:8080/loadmoremessages', {
    ...
    if (!response.ok) {
      ...
      throw new Error('No response after get LoadMoreMessages');
    } else {
      root.removeChild(modal);
    ...
    return 'Catch error in LoadMoreMessages';
  }
};
